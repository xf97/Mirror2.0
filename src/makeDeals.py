...
def getNormalListBias(_low, _high, _loc, _bias, _coolingValue, _handCountValue, _thisYearPrice, _baseDiff):
	...
	if _baseDiff >= 1.1:
		...
		_baseDiff = 0.9
	_bias = _bias * _coolingValue * _handCountValue * (1 / (USERS_NUM ** 2)) * _baseDiff
	sigma = _loc * 0.2 / 6
...
def getPrice(_low, _high, _price, _bias, _coolingValue, _handCountValue, _thisYearPrice, _baseDiff):
	...
	#生成符合正态分布的数列
	#要抑制偏移的幅度
	buyPriceList = getNormalListBias(_low, _high, _price, _bias, _coolingValue, _handCountValue, _thisYearPrice, _baseDiff)
	count = 0
	...
	sellPriceList = getNormalListBias(_low, _high, price1, _bias, _coolingValue, _handCountValue, _thisYearPrice, _baseDiff)
	count = 0
	while len(sellPriceList) == 0:
		#print("\r计算正态分布中...", end = "")
		count += 1
		sellPriceList = getNormalListBias(_low, _high, _price, _bias, _coolingValue, _handCountValue, _thisYearPrice, _baseDiff)
		if count >= 10000:
			while len(priceList) <= 100:
				tempNum = random.uniform(_low, _high + LOSS_VALUE)
				if tempNum >= _high:
...
def doTransaction(_accountsList, \
	...
	user1Price, user2Price = getPrice(lowLimit, highLimit, price, _normalVolume[_shareIndex] , coolingValue, handCountValue, thisYearPrice, _sharesList[_shareIndex].getBaseDiff())
